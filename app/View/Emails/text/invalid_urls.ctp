...
We have noticed, that information URLs of one or more courses of yours in
...
As we want to prevent showing dead links to the audience,
please have a look at the listed erroneous records and update the information soon.
Please review all data thoroughly and do not forget to submit the form.
The validation logic will highlight all found errors.
Additional options will appear, to get an
...
the "skip url validation" checkbox. This choice will be stored until the
course record's valid date has expired.

If you are currently not logged in to the DH Course Registry, you will be redirected to
the login form before accessing the course edit form.
...
foreach($data as $id => $course) {
	...
	echo "Course: \n".$course['Course']['name']."\n";
	echo "View on homepage: ";
	echo Configure::read('dhcr.baseUrl').'courses/view/'.$id."\n";
	echo "Edit course: ";
